                        ...
                        if (prng.Next(0, 100) < currentMap.foliageRate * 100)
                        {
                            ...
                            for (int f = 0; f < regions[i].Foliage.Length; f++)
                            {
                                if (foliageRandomNumber <= regions[i].Foliage[f].cumulativeWeight)
                                {
                                    ...
                                    if (f == 1)
                                    {
                                        newFoliage.name = "Tree";
                                        newFoliage.GetOrAddComponent<BoxCollider2D>();
                                        newFoliage.GetComponent<BoxCollider2D>().size = new Vector2(0.6f, 1);
                                        newFoliage.GetComponent<BoxCollider2D>().offset = new Vector2(0f, 0.4f);
                                        newFoliage.GetOrAddComponent<Rigidbody2D>();
                                        newFoliage.GetOrAddComponent<Rigidbody2D>().isKinematic = true;

                                        newFoliage.tag = "Tree";
                                        //newFoliage.localScale = Vector3.one * 2;
                                    }
